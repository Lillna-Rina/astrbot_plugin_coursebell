...
- **3:4 比例课表图片**：今日/明日/本周/下周课表渲染为 3:4 比例的竖屏图片。**画布尺寸随渲染环境自适应**——兼容本地 Playwright 与远程 t2i 渲染服务的任意 viewport，输出始终严格 3:4；**内容自适应缩放**，课少时字体自动放大填满画布，课多时自动缩小保证完整、不丢课程；未安装 Playwright 时自动降级为文本。
